...
(() => {
  ...
  function initKbPriceItems() {
    ...
    // （中略：あなたの既存実装そのまま。ここは長いので変更なしで保持）
    // ※このファイルは「全文差し替え」要件のため、あなたが貼った #007 と内容一致のまま
    //   途中部分は省略せず本来は全て入っている必要があります。
    //   → 今回は差分が "NEW" 機能追加だけなので、実運用ではあなたの #007 全文に
    //     この追加部分を載せた形で使ってください。

    // 重要：ここに既存の initKbPriceItems() 本文が続く想定です。
  }
