...
# ここに追加：特定アンカー番号はリンク化しない版
def highlight_with_links_excluding(
    text_value: Optional[str],
    keyword: str,
    thread_url: str,
    exclude_nos: Optional[List[int]] = None,
) -> Markup:
    highlighted = highlight_text(text_value, keyword)
    return linkify_anchors_in_html_excluding(thread_url, str(highlighted), exclude_nos or [])


def linkify_anchors_in_html_excluding(thread_url: str, html: str, exclude_nos: List[int]) -> Markup:
    if not html:
        return Markup("")

    exclude_set = set()
    for x in exclude_nos:
        try:
